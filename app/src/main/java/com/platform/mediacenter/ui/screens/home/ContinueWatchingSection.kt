...
@Composable
fun ContinueWatchingSection(navController: NavHostController, viewModel: HomeViewModel = hiltViewModel()) {

    var continueWatchingList by remember {
        mutableStateOf<List<HomeContentResponse.WatchedHistory?>>(
            emptyList()
        )
    }

    val result by viewModel.homeContent.collectAsState()
    continueWatchingList = result.data?.watchedHistory ?: emptyList()
    ...
    if (continueWatchingList.isNotEmpty()) {
        ...
        LazyRow(
            ...
            itemsIndexed(
                ...
                key = { _, item -> item?.title.hashCode() }) { _, item ->
                ...
                ContinueWatchingItemBox(
                    title = item?.title.toString(),
                    image = painter,
                    progress = 0.75f
                ) {
                    Log.e("4718", "ContinueWatchingSection: ")
                }
            }
        }
    }
